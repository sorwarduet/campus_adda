    ...
    useEffect(() => {
        ...
        onValue(messageRef, (snapshot) => {
            ...
            snapshot.forEach((item) => {
                if (item.val().senderId === data.uid && item.val().receiverId===activeInfo?.id || item.val().receiverId === data.uid && item.val().senderId===activeInfo?.id) {
                    ...
                    <div className="img_name">
                        <div className="img">
                            {activeInfo.status==='group'?
                                <h1 className="flex items-center justify-center mt-2 text-4xl text-red-600 font-bold ">{activeInfo.name[0]}</h1>
                            :
